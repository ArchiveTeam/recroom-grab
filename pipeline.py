...
LUA_SHA1 = get_hash(os.path.join(CWD, 'recroom.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'image':
                wget_args.extend(['--warc-header', 'recroom-image: '+item_value])
                wget_args.append('https://api.rec.net/api/images/v4/'+item_value)
            elif item_type == 'room':
                wget_args.extend(['--warc-header', 'recroom-room: '+item_value])
                wget_args.append('https://rooms.rec.net/rooms/'+item_value)
            elif item_type == 'account':
                wget_args.extend(['--warc-header', 'recroom-account: '+item_value])
                wget_args.append('https://accounts.rec.net/account/bulk?id='+item_value)
            elif item_type == 'event':
                wget_args.extend(['--warc-header', 'recroom-event: '+item_value])
                wget_args.append('https://apim.rec.net/apis/api/playerevents/v1/'+item_value)
            elif item_type == 'img':
                url = 'https://img.rec.net/' + item_value
                wget_args.extend(['--warc-header', 'recroom-img: '+url])
                wget_args.append(url)
            elif item_type == 'asset':
                url = 'https://' + item_value
                wget_args.extend(['--warc-header', 'recroom-asset: '+url])
                wget_args.append(url)
            else:
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/7/7e/Recroom-icon.png" height="50px" title=""/>
        <h2>Rec Room <span class="links"><a href="https://rec.net/">Website</a> &middot; <a href="https://tracker.archiveteam.org/recroom/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Rec_Room">Wiki</a></span></h2>
        <p>Archiving public Rec.net images, rooms, accounts, events, and assets.</p>
    '''
)
